...
function shortcode_recent_posts($atts, $content = null) {
		...
		foreach($latest as $post) {
				...
				if (($type=='team') || ($type=='services')) {
					if (($i%4)==0) { $addclass='no-margin'; }
				}
				
				if ($type=='history') {
					if ($j==$num)  { $addclass1='last'; }
					if (($k%3)==0) { $addclass2='no-margin'; }
				}	
				
				$custom = get_post_custom($post->ID);
				$teamjob = $custom["team-job"][0];
				$historyear = $custom["history-year"][0];
				$customurl = $custom["custom-url"][0];

				$output .= '<li class="entry '.$addclass.' '.$addclass1.' '.$addclass2.'">';
				
				if ($thumb == 'true') {
					if ( has_post_thumbnail($post->ID) ){
						if ($customurl=="") {
							$output .= '<div class="thumb-wrap"><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
							$output .= get_the_post_thumbnail($post->ID, "$thumb_size", array( "class" => "thumb" ));
							$output .= '</a></div>';
						} else {
							$output .= '<div class="thumb-wrap"><a href="'.$customurl.'" title="'.get_the_title($post->ID).'">';
							$output .= get_the_post_thumbnail($post->ID, "$thumb_size", array( "class" => "thumb" ));
							$output .= '</a></div>';
						}	
					}
						...
						if ($type=='history') {
							if ($historyear!="") {	
								$output .= '<span class="year">'.$historyear.'</span>';
							}	
						}
						...
						if ($type=='team') {
								$output .= '<span class="job">'.$teamjob.'</span>';
						}
						
						if($meta == 'true'){
								$output .= '<span class="meta">';
										$output .= __('Dodano: ', 'theme1407');
				...
				$output .= '</li><!-- .entry (end) -->';
				$i++; $j++; $k++; $addclass==""; $addclass1=""; $addclass2="";
		}
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1407");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1407");
